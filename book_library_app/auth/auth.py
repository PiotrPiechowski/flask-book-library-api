...
def login(args: dict):
    ...
    return jsonify({
        'success': True,
        'token': token #jwt.decode(token, current_app.config.get("SECRET_KEY"), algorithms=["HS256"])
    })

@auth_bp.route('/me', methods=['GET'])
@token_required
def get_current_user(user_id: int):
    user = User.query.get_or_404(user_id, description='User with id {user} not found')
